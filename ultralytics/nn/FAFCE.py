...
def carafe(x, normed_mask, kernel_size, group=1, up=1):
        ...
        # assert m_c == kernel_size ** 2 * up ** 2prin
        assert m_h == up * h
        assert m_w == up * w
        pad = kernel_size // 2
        # print(pad)
...
def hamming2D(M, N):
    ...
    Parameters:
    - M: Number of rows in the window
    - N: Number of columns in the window

    Returns:
    - 2D Hamming window
"""
# Generate Hamming windows in the horizontal and vertical directions

    # hamming_x = np.blackman(M)
    # hamming_x = np.kaiser(M)
    hamming_x = np.hamming(M)
    hamming_y = np.hamming(N)
    # Two-dimensional Hamming Windows are generated by outer product
...
def compute_similarity(input_tensor, k=3, dilation=1, sim='cos'):
    """
    Compute the cosine similarity between each point in the input tensor 
    and its surrounding points within a KxK neighborhood.

    Parameters:
    - input_tensor: Input tensor of shape [B, C, H, W]
    - k: Neighborhood size, indicating a KxK region around each point

    Returns:
    - Output tensor of shape [B, KxK-1, H, W]
    """

    B, C, H, W = input_tensor.shape
    # Use zero padding to handle boundary cases
    # padded_input = F.pad(input_tensor, (k // 2, k // 2, k // 2, k // 2), mode='constant', value=0)

    # Flattens every point in the input tensor and its surrounding points in the KxK range
    ...
    # Remove the cosine similarity of the center point and get the result of [KxK-1]
    similarity = torch.cat((similarity[:, :k * k // 2], similarity[:, k * k // 2 + 1:]), dim=1)

    # Shape the result back to [B, KxK-1, H, W]
